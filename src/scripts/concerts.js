...
function fetchData(genre_Id) {
  fetch(
    `https://app.ticketmaster.com/discovery/v2/events.json?city=nashville&genreId=${genre_Id}&sort=date,asc&apikey=${ticketmasterKey}`
  )
...
function getConcerts(results) {
  ...
  makeCategory.textContent = "Concerts";
  ...
  for (let i = 0; i < 5; i++) {
    ...
    renderSearchResults(artist, venue, "concerts");
  }
}
